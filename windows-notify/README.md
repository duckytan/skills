# Windows 气球通知 (Windows Balloon Notifications)
...
将整个 `windows-notify` 文件夹复制到您的 Claude AI 的 Skills 目录中，例如：
...
```bash
# 基本语法
node windows-notify/scripts/index.js --title "<标题>" --message "<内容>" --icon <图标> --duration <时长(毫秒)>

# 示例用法
node windows-notify/scripts/index.js --title "完成" --message "导出已成功完成" --icon info

node windows-notify/scripts/index.js --title "警告" --message "系统资源占用过高" --icon warning

node windows-notify/scripts/index.js --title "错误" --message "连接失败，请检查网络" --icon error 

node windows-notify/scripts/index.js --title "提醒" --message "请完成今日任务清单" --icon none --duration 6000
```
