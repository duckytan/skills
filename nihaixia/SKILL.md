...
| 汉代/台湾/唐代度量衡换算 | references/sources/books/人纪-4-伤寒论/ 搜"度量衡" | 经方剂量古今换算 |
...
| 文件 | 内容 | 用途 |
|------|------|------|
| `01-问诊十问.md` | 10 项问诊清单 + 工作流 + 失败模式 | **任何诊断必走**：收集用户症状 → 才能进六经辨证 |
| `02-体质先天判定.md` | 八字输出模板 + 解盘算法 + 5 字段→10 问对接表 + 应用示例 | **可选辅助**：用户主动提供公历生日时启用 |
...
**调用顺序**：
1. 用户问中医诊断 → 先读 `01-问诊十问.md`，收齐 10 项问诊
2. 用户主动提供公历生日 → 可选读 `02-体质先天判定.md` + 跑 `scripts/bazi_paipan.py`
