...
| `free < 20 GiB`（绝对下限）但尚未停机 | **同样先清一次回收站 → 复测**（v3.7.9 起两门对称，见下行）|
| 清完仍不足 | `fail_reason = DISK_GUARD_SKIP`，`SKIPPED`，**继续处理队列下一个，不停机** |
| 任意时刻 `free < 20 GiB`（绝对下限 `MIN_FREE_BYTES`） | **先清一次回收站 → 复测**；复测后仍 `< 20 GiB` 才**立即停机**，写 `batches.status=ABORTED`，保留现场等用户处理 |

> 修订（v3.7.9）：绝对下限原先**不试回收站清理就直接停机**（与上一行 `free < need` 的补救不对称），
> 而本环境删源包全部进 `$RECYCLE.BIN`、回收站里常年堆着几十 GB 本流水线自己的条目 ——
> 于是连续 6 个批次（09-09 / 09-11 / 09-15 / 09-16×4 / 09-17）共 7 次**无谓中止**，
> 每次都在「清一次回收站就够继续」的情况下白停整批。修复后两门对称：都先清一次回收站再复测。
> **阈值 `MIN_FREE_BYTES` 未改** —— 修的是「把可回收空间收回来」，不是「把闸门放宽」。
...
运行期：每解完一个包 → 立刻删源包（★ 目的只有一个：目录干净。不要指望它腾空间）
        空间闸门算 free 时【不】把回收站计入可用；free < need **或** free < MIN_FREE_BYTES(20GiB) 时都先清一次回收站后复测（v3.7.9 起对称）
